...
export default function GameBoard({
  ...
  const getCardOne = (data) => {
    return getThemeData(data, theme[2], theme[3]).find((word) => {
  ...
  const unlockLevel = (e) => {
    ...
    handleLevels({ ...levels, [theme[2]]: [theme[3]] });
  };
  ...
  useEffect(() => {
    if (Number(theme[3]) === 3 && notMatched === false) {
      unlockLevel();
    }
  }, [notMatched]);
  ...
  if (!notMatched && cards.length !== 0 && Number(theme[3]) !== 3) {
    return (
      <div>
        <button
  ...
  return (
    <div className={"endDiv"}>
      <h1>Gratulujeme!</h1>
      <h2>Dokončil jsi téma: {themes[theme[2]]}</h2>
